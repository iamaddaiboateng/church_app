...
class BibleStudyController with ChangeNotifier {
  Box bibleStudyBox = Hive.box(kHiveBibleStudyBox);
  Box memberShipBox = Hive.box(kHiveChurchDatabase);

  String bibleStudyName = '';
  List cellMembers = [];
  BibleStudyModel homeCellModel = BibleStudyModel();

  Future<void> addHomeCell(BibleStudyModel model) async {
    bibleStudyBox.add(model.toMap()).then((value) {
  ...
  deleteHomeCell(index, String cellName) {
    Get.defaultDialog(
      ...
      actions: [
        TextButton(
          onPressed: () {
            bibleStudyBox.deleteAt(index).whenComplete(
              () {
                Get.back();
  ...
  updateHomeCellData(key, BibleStudyModel model) {
    bibleStudyBox.put(key, model.toMap()).whenComplete(() {
      Get.back();
  ...
  getCellMembers(String cellName) {
    cellMembers = memberShipBox.values
        .where((element) =>
            element['Bible Study'].toString().toLowerCase() ==
