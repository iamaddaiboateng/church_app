...
class CellMeetingsReport extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.all(10.0),
      child: MaterialContainer(
        info: 'Cell Meeting Report',
        child: Scaffold(
          body: ValueListenableBuilder(
            valueListenable: Hive.box(kHiveCellMeetings).listenable(),
            builder: (_, Box box, __) {
              List meetings = box.values
                  .where(
                    (element) =>
                        element[HomeCellString.cellName] ==
                        Provider.of<HomeCellController>(context).cellName,
                  )
                  .toList();
